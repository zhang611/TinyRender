...
bool TGAImage::drawline(int x0, int y0, int x1, int y1, TGAColor color) {
    // first attempt(version 1):
    //  for (float t=0.; t<1.; t+=.01) {
    //     int x = x0 + (x1-x0)*t;
    //     int y = y0 + (y1-y0)*t;
    //     if(!set(x, y, color)) return false;
    // }

    // second attempt(my logic):
    //  int xlen = std::abs(x0 - x1), ylen = std::abs(y0 - y1);
    //  //if line is steep . we use dy, or dx
    //  float dt = xlen < ylen ? 1/(float)ylen : 1/(float)xlen;
    ...
    // third attempt(version 3):
    //  int dx = x1 - x0, dy = y1 - y0;
    //  float derror = std::abs(dy/float(dx)), error = 0;
    //  int y = y0;
    ...
    // fourth attempt(version 4： no float, no double, no multiply, no
    // division):
    int dx = x1 - x0, dy = y1 - y0;
